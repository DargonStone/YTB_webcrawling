...
## 정보를 입력받을 변수
file_name = list(input("저장할 파일 이름을 차례대로 띄어쓰기를 기준으로 정해주세요 Ex) 벤츠 현대 :  ").split())
file_name_count = int(0)
# 영상 주제 들어갈 리스트
ytb_title_name = list(input('동영상 주제를 차례대로 띄어쓰기를 기준으로 정해주세요 Ex) 벤츠 현대 :  ').split())
...
for x in ytb_title_name:
    ...
    if i == ytb_video_count-1:
        ...
        df.to_csv("%s.txt" % file_name[file_name_count], encoding="utf-8-sig")
        print("%d 개의 작업이 안료 되었습니다." % len(ytb_comment_list))

        file_name_count += 1
        ## 다시 정보를 넣기 위해서 리스트 url과 댓글 리스트 초기화
        url_list.clear()
        ytb_comment_list.clear()
